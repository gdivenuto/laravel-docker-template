...
$ruta_directorio_fotos = RUTA_FOTOS_GACETILLAS;
...
if ( isset($_POST["upload"]) )
{
	...
	if (strlen($mensaje_error) == 0) {
		...
		if (isset($_FILES['image']['name'])) {
			...
			if ($width > GACETILLA_ANCHO_MAXIMO_FOTO_A_RECORTAR) {
				$escala   = GACETILLA_ANCHO_MAXIMO_FOTO_A_RECORTAR / $width;
...
if ( isset($_POST["upload_recorte"]) )
{
	...
	$escala = ($w != '') ? GACETILLA_ANCHO_FOTO_RECORTE / $w : GACETILLA_ANCHO_FOTO_RECORTE;
	...
	// Se registra el nombre de la foto de la Gacetilla en la base de datos
...
<html lang="es" >
	...
	<body>
		<div>
			<?php
			$ruta_completa_foto_a_recortar = $ruta_directorio_fotos."normal_".$codigo.".".$extension;
			$url_completa_foto_a_recortar = URL_FOTOS_GACETILLAS."normal_".$codigo.".".$extension;
			...
			if ( file_exists($ruta_completa_foto_a_recortar) )
			{
				...
				<script type="text/javascript">
					...
					{ 
						var scaleX = <?= GACETILLA_ANCHO_FOTO_RECORTE; ?> / selection.width; 
						var scaleY = <?= GACETILLA_ALTO_FOTO_RECORTE; ?> / selection.height; 
			...
			if ( $nombre_foto != '' )
			{
				...
				if ( isset($resultado_carga_foto) && $resultado_carga_foto == 1 ) { ?>
					<script>
						top.location.href = '<?=URL_ABMS;?>?controlador=gacetillas&accion=editar&id=<?=$codigo;?>';
					</script>
				<?php
				}
			...
			else // Si NO posee foto
			{
				...
				?>
					<div class="p_edicion_modal_botones">
						...
						<div id="btnCancelar" class="p_boton_en_modal_foto">
					...
					<script type="text/javascript">
						$(document).ready(function () {
							...
							function addImgAreaSelect( img ) {
						        ...
						        img.load(function() {
						        	// Se calcula la altura proporcional
				                    var height = this.width / 16 * 9;

				                    // Si la altura calculada es menor o igual a la de la foto
				                    if( height <= this.height ) { 
				                    	// Se calcula la diferencia entre las alturas
			                            var diferencia = ( this.height - height ) / 2;
			                            // Se definen las coordenadas
			                            var coordenadas = { x1 : 0, y1 : diferencia, x2 : this.width, y2 : height + diferencia };
				                    }   
				                    else // si la altura está fuera de los límites
				                    {
				                    	// Se calcula el ancho proporcional
			                            var width = this.height / 9 * 16;
			                            // Se calcula la diferencia entre los anchos
			                            var diferencia = ( this.width - width ) / 2;
			                            // Se definen las coordenadas
			                            var coordenadas = { x1 : diferencia, y1 : 0, x2 : width + diferencia, y2: this.height };
				                    }

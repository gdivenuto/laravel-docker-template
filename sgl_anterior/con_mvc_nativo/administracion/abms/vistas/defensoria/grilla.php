...
class VistaInscripcionesGrilla extends VistaBase {
	...
	public function mostrar($datos, $mensaje = '', $tipo_mensaje = '', $filtro) {
		...
		$url_observaciones = URL_ABMS.'?controlador=defensoria_observacion&accion=listar';
		?>
		<!DOCTYPE html>
		<html lang="es">
		  	<head>
			    ...
			    <div class="container-fluid p-0 px-3">
					...
					<div class="row">
						<div class="col fuente_titulos bg-dark text-white small py-1 titulo_entidad">
							Inscripciones <strong>a Moderar</strong> para Defensor del Pueblo
						</div>
					</div>
						...
						<div class="row mt-1">
							<div class="col-12 col-md-3 mt-1 mt-md-0">
								<input  type="text" name="f_texto" id="f_texto"
										...
										class="form-control form-control-sm w-100 small" placeholder="Por Solicitante, DNI o mail...">
							</div>
							...
							<div class="col-12 col-md-2 mt-1 mt-md-0">
								<div class="form-group form-control-sm small form-check pt-1">
									...
									<label class="form-check-label" for="chk_habilitados">S&oacute;lo habilitados</label>
							...
							<div class="col-12 col-md-5 mt-1 mt-md-0">
								...
								<?php if ($cantidad > 0) {?>
									...
									<a  href="<?=$url_observaciones;?>"
										class="btn btn-info btn-sm mt-1 mt-md-0"
										title="Observaciones realizadas">
										<i class="fas fa-edit"></i>&nbsp;Ver Observaciones
									</a>
								<?php } ?>
							</div>
						</div>
									...
									<tbody>
										...
										<tr <?=($dato['habilitado'] == '0') ? ' class="text-muted"' : '';?> >
											<td class="text-center">
												<a  href="javascript:refrescarEnModal('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=mostrarDocumentos&dni=<?=$dato['dni'];?>');" 
													title="Ver documentos">
													<i class="far fa-file-pdf"></i>
												</a>
											</td>
											<!-- <td class="text-center">
												<a  href="<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=generarFicha&id=<?=$dato['id'];?>" 
													target="_blank" 
									        ...
									        <td>
									        	<?=($dato['telefono']) ? $dato['telefono'] : '&nbsp;';?>
									        </td>
									        <td>
									        	<?=($dato['email']) ? $dato['email'] : '&nbsp;';?>
									        </td>
									        <td>
									        ...
									        <td class="text-center">
												<a  href="javascript:refrescarEnModal('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=editarHabilitacion&id=<?=$dato['id'];?>');" 
													title="Habilitaci&oacute;n de Candidato">
													<?= ($dato['habilitado'] == '1' 
											...
											<td class="text-center">
											    <a  href="<?=$url_observaciones;?>&candidato_id=<?=$dato['id'];?>"
													title="Observaciones realizadas al Candidato">

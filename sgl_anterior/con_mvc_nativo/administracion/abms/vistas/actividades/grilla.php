...
class VistaActividadesGrilla extends VistaBase {
	...
	public function mostrar($datos, $mensaje = '', $tipo_mensaje = '', $filtro) {
		...
		$criterio_buscador .= "&f_contenido=" . $filtro['f_contenido'];
		...
		<html lang="es">
		  	...
		  	<body>
						...
						<div class="form-row">
							<div class="col-12 col-md-2 mt-1">
								<input  type="text" name="f_titulo" id="f_titulo"
										value="<?=($filtro['f_titulo']) ? $filtro['f_titulo'] : '';?>"
										class="form-control form-control-sm small"
										placeholder="T&iacute;tulo">
							</div>
							<div class="col-12 col-md-4 mt-1">
								<input  type="text" name="f_contenido" id="f_contenido"
										value="<?=($filtro['f_contenido']) ? $filtro['f_contenido'] : '';?>"
										class="form-control form-control-sm small"
								...
								<div class="table-responsive">
									<table class="table table-hover table-bordered table-sm small">
										...
										<?php for ($i = 0; $i < $cantidad; $i++) {$dato = &$datos[$i];?>
											<tr <?=($dato['a_habilitado'] == '0') ? ' class="text-muted"' : '';?> >

										    	<td class="text-center" width="30">
										    		<a href="javascript:redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=editar&id=<?=$dato['a_codigo'];?>&pagina=<?=$filtro['pagina'];?>');" title="Editar registro">
										    			<i class="fas fa-edit"></i>
										    		</a>
										    	</td>

										    	<td class="text-center" width="30">
										    		<a href="javascript:if(confirm('¿Desea eliminar el registro?')){redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=eliminar&id=<?=$dato['a_codigo'];?>&pagina=<?=$filtro['pagina'];?>');};" title="Eliminar registro">
										    			<i class="fas fa-trash"></i>
										    		</a>
										    	</td>

												<td class="text-center" width="80">
													<?=$this->formatearFecha($dato['a_fecha']);?>
												</td>

										        <td class="text-center" width="30">
										        	<?=($dato['a_hora']) ? $dato['a_hora'] : '&nbsp;';?>
										        </td>

												<td>
													<?=($dato['a_titulo']) ? $dato['a_titulo'] : '&nbsp;';?>
												</td>

												<td>
													<?=($dato['a_contenido']) ? $dato['a_contenido'] : '&nbsp';?>
												</td>

												<td class="text-center" width="40">
													<?php if ($dato['a_habilitado'] == '1') {?>
														<a title="Deshabilitar registro" href="javascript:if(confirm('¿Desea deshabilitar el registro?')){redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['a_codigo'];?>&habilitado=<?=$dato['a_habilitado'];?>&pagina=<?=$filtro['pagina'];?>');};">
															<i class="fas fa-check"></i>
														</a>
													<?php } else {?>
														<a title="Habilitar registro" href="javascript:redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['a_codigo'];?>&habilitado=<?=$dato['a_habilitado'];?>&pagina=<?=$filtro['pagina'];?>');">
			    ...
			    <script src="<?=URL_JS;?>actividades/grilla.js?v=<?=date("Ymd_His");?>"></script>
		  	</body>
		</html>
		<?php }
}

...
class VistaParticipacionesFichaPDF extends VistaBase {
	...
	public function mostrar($dato) {
				...
				<tbody>
					<tr>
						<td colspan="2" class="pdf_ficha_expediente pdf_texto_gris">
							Participaci&oacute;n N&deg; <?=$dato['numero_participacion'];?> en <?=$documento;?>&nbsp;
							<strong><?=sprintf('%d&nbsp;%s&nbsp;%d&nbsp;%d&nbsp;%d', $dato['anio'], $dato['tipo'], $dato['numero'], $dato['cuerpo'], $dato['alcance']);?></strong>
						</td>
					</tr>
					<tr>
						<td class="pdf_pt_10">
		...
		try {
			...
			$html2pdf->Output("ficha_participacion.pdf");

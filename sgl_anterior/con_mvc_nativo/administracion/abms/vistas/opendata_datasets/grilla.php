...
class VistaOpendataDatasetsGrilla extends VistaBase {
	...
	public function mostrar($datos, $mensaje = '', $tipo_mensaje = '', $filtro) {
		...
		<html lang="es">
		  	...
		  	<body>
						...
						<div class="form-row">
							<div class="col-12 col-md-2 mt-1">
								<input type="text" name="f_titulo" id="f_titulo"
										value="<?= ($_SESSION['filtro_opendata_datasets']['f_titulo']) ? $_SESSION['filtro_opendata_datasets']['f_titulo'] : ''; ?>" 
										class="form-control form-control-sm small"
										placeholder="T&iacute;tulo">
							</div>
							<div class="col-12 col-md-4 mt-1">
								<input  type="text" name="f_descripcion" id="f_descripcion" 
										value="<?= ($_SESSION['filtro_opendata_datasets']['f_descripcion']) ? $_SESSION['filtro_opendata_datasets']['f_descripcion'] : '';?>" 
										class="form-control form-control-sm small" 
							...
							<div class="col-12 col-md-2 mt-1">
								<input  id="f_fecha" name="f_fecha"
										...
										value="<?=(isset($f_fecha)) ? $this->formatearFecha($f_fecha) : '';?>"
										placeholder="Fecha" />
							</div>
									...
									<table class="table table-hover table-bordered table-sm small">
										<thead class="thead-light">
											<tr>
												...
												<th class="text-center">Cat&aacute;logo</th>
												<th class="text-center">Publicador</th>
										...
										<?php for ($i = 0; $i < $cantidad; $i++) {$dato = &$datos[$i];?>
											<tr <?=($dato['habilitado'] == '0') ? ' class="text-muted"' : '';?> >
										        ...
										        <td><?= $this->formatearFecha($dato['fecha_emitido']); ?></td>

												<td><?= $this->formatearFecha($dato['fecha_modificado']); ?></td>

												<td><?= ($dato['titulo']) ? $dato['titulo'] : '&nbsp;'; ?></td>

												<td><?= ($dato['nombre_catalogo']) ? $dato['nombre_catalogo'] : '&nbsp;'; ?></td>

												<td><?= ($dato['nombre_publicador']) ? $dato['nombre_publicador'] : '&nbsp;'; ?></td>

												<td class="text-center" width="40">
													<?php if ($dato['habilitado'] == '1') {?>
														<a title="Deshabilitar registro" href="javascript:if(confirm('¿Desea deshabilitar el registro?')){redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['id'];?>&habilitado=<?=$dato['habilitado'];?>&pagina=<?=$filtro['pagina'];?>');};">
															<i class="fas fa-check"></i>
														</a>
													<?php } else {?>
														<a title="Habilitar registro" href="javascript:redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['id'];?>&habilitado=<?=$dato['habilitado'];?>&pagina=<?=$filtro['pagina'];?>');">
			    ...
			    <script src="<?=URL_JS;?>opendata_datasets/grilla.js?v=<?=date("Ymd_His");?>"></script>
		  	</body>
		</html>
		<?php }
}

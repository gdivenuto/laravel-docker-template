...
require_once RUTA_MODELOS."auditoria_personal.php";

//Incluye la vista que corresponde
require_once RUTA_VISTAS . "auditoria_personal/grilla.php";

class auditoria_personal_controller extends ControllerBase
{
	public function __construct()
	{
		...
		$this->modelo = new auditoriaPersonalModel();

		// Se crea una instancia de cada Vista
		$this->vista_grilla = new VistaAuditoriaPersonalGrilla();
	}
	...
	public function listar()
    {
		...
		// POR LEGAJO
		$filtro['f_legajo'] = LibreriaGeneral::recoge('f_legajo');
    ...
    public function asignarLegajoAuditoriaPersonal()
    {
		$legajos = Array();
		
		// SE OBTIENEN LAS OBSERVACIONES QUE CONTENGAN "...egajo..."
		$listado = $this->modelo->obtenerObservaciones();
		
		$cant_listado = count($listado);
		// PARA CADA OBSERVACION
		for( $i=0; $i < $cant_listado; $i++) {
			// SE EXTRAE EL LEGAJO DE LA OBSERVACION
			$legajos[$i] = $this->modelo->extraerLegajo($listado[$i]['observaciones_log']);
			
			if ( !$this->modelo->asignarLegajo($legajos[$i], $listado[$i]['id_log']) ) {
				return false;
			}
		}
	}

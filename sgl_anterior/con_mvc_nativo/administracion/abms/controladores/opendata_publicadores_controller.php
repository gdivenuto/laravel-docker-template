...
require_once RUTA_VISTAS . "opendata_publicadores/grilla.php";
require_once RUTA_VISTAS . "opendata_publicadores/edicion.php";

class opendata_publicadores_controller extends ControllerBase
{
	public function __construct() {
		...
		$this->modelo = new opendataPublicadoresModel();
	
		// Se crea una instancia de cada Vista
		$this->vista_grilla = new VistaOpendataPublicadoresGrilla();
		$this->vista_edicion = new VistaOpendataPublicadoresEdicion();
    ...
    public function listar($mensaje = '', $tipo_mensaje = '')
	{
		...
		$_SESSION['filtro_opendata_publicadores'] = $filtro;
    ...
    public function insertar() {
		...
		if ( $this->modelo->existe($datos['titulo'], $datos['fecha_emitido']) )
			$this->listar('Ya se ha ingresado un Publicador con dicho titulo y fecha.', 2);
		elseif ($this->modelo->insertar($datos))
			$this->listar('Se agreg&oacute; con &eacute;xito el Publicador.', 1);
		else
			$this->listar('Error al agregar el Publicador.', 2);
    }

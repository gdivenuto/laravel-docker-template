...
'.$content.'
         ...
         * Calcula la posición de inicio de la siguiente línea, dependiendo del text-align
         *
         * @access protected
         * @param  integer $curr real current position in the text, if new line in the write of a text
         */
        protected function _setNewPositionForNewLine($curr = null)
        {
            ...
            // Obtiene los márgenes de la línea actual
            ...
            // Si es justify => establece el espaciado de palabras
        ...
        protected function _makeHTMLcode()
        {
            ...
            for ($this->_parsePos=0; $this->_parsePos < count($this->parsingHtml->code); $this->_parsePos++) {
                ...
                if (in_array($action['name'], array('table', 'ul', 'ol')) && !$action['close']) {
                    ...
                    //  trabajaremos como un sub HTML para calcular el tamaño del elemento
                    $this->_subPart = true;

                    // get the name of the opening tag
                    // tomamos el nombre de la etiqueta abierta
                    $tagOpen = $action['name'];

                    // save the actual pos on the parsing
                    // se guarda la posición actual en el parseo
                    $this->_tempPos = $this->_parsePos;

                    // foreach elements, while we are in the opened tag
                    // para cada elemento, mientras tengamos la etiqueta abierta
                    ...
                    // se ejecuta el cierre de la etiqueta
        ...
        protected function _executeAction($action)
        {
            ...
            // si es la primera acción de la primera página, y si no es una etiqueta abierta de PAGE => crea la página nueva
        ...
        /**
         * [_wrapperExecuteAction description]
         * @param  [type] $action [description]
         * @return [type]         [description]
         */
        protected function _wrapperExecuteAction($action) 
        {
            // tag of the action
            $action_tag = strtoupper($action['name']);

            $result = array();
            $result['normal_result'] = $this->_executeAction($action);
            $result['patch_result'] = false;

            if (in_array($action_tag, array('B', 'STRONG'))) {
                $result['patch_result'] = true;
                $result['normal_result'] = false;
            }
        ...
        {
            // la linea de abajo la agrego pduthey
            if ($this->parsingCss->value['page-break-after'] == "always") { $this->_setNewPage(null, '', null, $this->_defaultTop); $this->parsingCss->setPosition(); }
         ...
         * esto no es un TAG real, solo es para escribir el texto
         *
         * @param  array $param
         * @return boolean
         */
        protected function _tag_open_WRITE($param)
        {
            ...
            // PROBANDO !!!
            //if ($txt == 'DEPURACION') {
            //    Logger::getinstance()->Log("parsingCss_value", $this->parsingCss->value, false);
            //}

            // Si is just after a float
            if ($this->_isAfterFloat) {
                $txt = ltrim($txt);
                $this->_isAfterFloat = false;
            }

            // Se reemplazan en el texto los valores de Número y Cantidad de páginas
            $txt = str_replace('[[page_nb]]', $this->pdf->getMyAliasNbPages(), $txt);
            $txt = str_replace('[[page_cu]]', $this->pdf->getMyNumPage($this->_page), $txt);

            // Si está definida la propiedad, se modifica la capitalización respectiva
            ...
            // PROBANDO !!!
            // size of the text
            $h  = 1.08*$this->parsingCss->value['font-size'];// altura = 80% del tamaño de fuente
            ...
            // calcula el ancho de cada palabra, y de toda la oración
            $w = 0;
            $words = explode(' ', $txt);
            ...
            // mientras tenemos palabras, y el texto no cabe en la línea => cortamos la oración
            while ($x+$w>$right && $x<$right+$space && count($words)) {
                ...
                // si nada encaja en la línea, y si la primera palabra no cabe en la línea => la palabra es demasiado larga, la ponemos
            ...
            // si tenemos palabras después del corte automático, es porque encajan en la línea => escribimos el texto
            // 
            if (count($words)) {
                $txt = '';
                // por cada palabra

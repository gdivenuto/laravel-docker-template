...
class VistaRemitenteGrilla extends VistaBase {
			    ...
			    <div class="container-fluid p-0 px-3">
					...
					<div class="row mt-1">
						<div class="col-md-12">
										    	...
										    	<?php if ( in_array($_SESSION['perfil6'], [1,2]) ) { ?>
													<td class="text-center" width="40">
														<?php if ($dato['habilitado'] == '1') {?>
															<a title="Deshabilitar Remitente" href="javascript:if(confirm('¿Desea deshabilitar el Remitente?')){redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['id'];?>&habilitado=<?=$dato['habilitado'];?>&pagina=<?=$filtro['pagina'];?>');};">
																<i class="fas fa-check"></i>
															</a>
														<?php } else {?>
															<a title="Habilitar Remitente" href="javascript:redireccionar('<?=URL_ABMS;?>?controlador=<?=$this->controlador;?>&accion=modificarEstado&id=<?=$dato['id'];?>&habilitado=<?=$dato['habilitado'];?>&pagina=<?=$filtro['pagina'];?>');">
							...
							<?php } else {?>
								<div class="alert alert-info small">No se han encontrado resultados.</div>
							<?php }?>
						</div>
					</div>
				</div>

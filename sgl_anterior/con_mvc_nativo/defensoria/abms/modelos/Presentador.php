...
class PresentadorModel extends ModeloBaseMySQLi {
	...
	private function getFiltro() {
		...
		if (isset($this->filtro['valor_buscado']) && $this->filtro['valor_buscado'] != '') {
			...
			return " AND ( P.nombre LIKE '%" . $criterio_ingresado . "%' OR
						   P.apellido LIKE '%" . $criterio_ingresado . "%' OR
						   P.localidad LIKE '%" . $criterio_ingresado . "%' OR
						   P.direccion_calle LIKE '%" . $criterio_ingresado . "%' OR
						   P.tel_fijo_numero LIKE '%" . $criterio_ingresado . "%' OR
						   P.movil_numero LIKE '%" . $criterio_ingresado . "%' OR
						   P.mail LIKE '%" . $criterio_ingresado . "%' OR
						   P.dni LIKE '%" . $criterio_ingresado . "%' OR
						   P.observaciones LIKE '%" . $criterio_ingresado . "%'
                         )";
		}
		return "";
	}
	...
	public function listar() {
		...
		$sql = "SELECT 
					P.*
				FROM " . $this->tabla_def_presentadores . " AS P
				WHERE P.habilitado <> 3
			    " . $this->getFiltro() . "
			    ORDER BY P." . $_SESSION['ultimo_campo'] . " " . $_SESSION['ultimo_sentido'] . "
	...
	public function obtenerCantidad() {
		...
		$query = "SELECT COUNT(P.id) AS cantidad 
				  FROM " . $this->tabla_def_presentadores . " AS P
				  WHERE P.habilitado <> 3 " . $this->getFiltro();
	...
	/**
	 * Se obtiene el listado, para el informe
	 * @return array $datos
	 */
	...
	public function obtenerUltimoId() {
		...
		$query = "SELECT MAX(id) AS ultimo_codigo FROM " . $this->tabla_def_presentadores;
	...
	public function obtenerRegistro($id = 0) {
		...
		$sql = "SELECT * FROM " . $this->tabla_def_presentadores . " WHERE id = " . $id;
	...
	public function existe($datos) {
		...
		$query = "SELECT nombre FROM " . $this->tabla_def_presentadores . " WHERE dni = " . $datos['dni'];
	...
	public function noLoModificoOtroUsuario() {
		...
		$query = "SELECT id
				  FROM " . $this->tabla_def_presentadores . "
	...
	public function insertar($datos) {
		...
		$query = "INSERT INTO " . $this->tabla_def_presentadores . "(
						nombre, 
						apellido, 
						dni,
						provincia_id, 
		...
		if (!$this->ejecutarQuery($query)) {
			...
			$this->auditarEnDefensoria(
				"ALTA",
				$this->tabla_def_presentadores,
				$this->obtenerUltimoId(),
				str_replace("'", "", "Se ingresa el Presentante " . $datos['nombre'] . " " . $datos['apellido'])
			);
		}
		return true;
	}
	...
	public function modificar($datos) {
		...
		$query = "UPDATE " . $this->tabla_def_presentadores . "
		...
		if (!$this->ejecutarQuery($query)) {
			...
			$this->auditarEnDefensoria(
				...
				$this->tabla_def_presentadores,
				$datos['id'],
				str_replace("'", "", "Se modifica el Presentante " . $datos['nombre'] . " " . $datos['apellido'])
			);
		}
		return true;
	}
	...
	public function eliminar($id) {
		...
		$query = "DELETE FROM " . $this->tabla_def_presentadores . " WHERE id = " . $id;
		...
		if (!$this->ejecutarQuery($query)) {
			...
			$this->auditarEnDefensoria(
				...
				$this->tabla_def_presentadores,
				$id,
				"Se elimina un Presentante con Id " . $id
			);
		}
		return true;
	}
	...
	public function listarHabilitados() {
		...
		$sql = "SELECT * FROM " . $this->tabla_def_presentadores . " 
				WHERE habilitado = 1 
	...
	public function obtenerInfo($nombre) {
		...
		$sql = "SELECT * FROM " . $this->tabla_def_presentadores . " WHERE habilitado = '1' " . $filtro;

<?php
/**
 * Clase ActuacionExpeElecRechazarFirma
 * 
 * Clase de actuación que se encarga de cancelar la firma firma de 
 * un documento electrónico perteneciente a un expediente digital.
 *
 * NOTA: actualmente la clase no posee persistencia en base de datos.
 * 
 */
class ActuacionExpeElecRechazarFirma extends Actuacion {
	// ************************************************************************
	...
	public function __construct()
	{
		...
		parent::__construct(
			...
			'Rechazar la solicitud de firma de un documento perteneciente a un expediente electrónico.'
		);
		...
		$this->agregarPaso('RechazarFirma', [
			'paso_ayuda' => '<strong>Usted está a punto de rechazar la solicitud de firma para el documento.</strong><br/>Este rechazo no alterará ni modificará el documento de ninguna manera, pero dejará una constancia al respecto en el sistema.',
		 	'rechazo_obligatorio' => true,
			'documento_a_rechazar' => [
				'tipo_documento' => 'directo', 
				'url_base_documento_a_rechazar' => URL_KRAKEN_RESOURCES_PROYECTOS,
			]
		]);
		...
		$this->agregarPaso('EditorTexto', [
			...
			'texto_placeholder' => 'Si lo desea, ingrese aquí el motivo de rechazo de la solicitud de firma del documento.',

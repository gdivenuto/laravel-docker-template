...
class Logger
{
    ...
    private function ObtenerArchivoSiguiente($identificador)
    {
    	...
    	$iterador = new GlobIterator(PATH_KRAKEN_LOG.$identificador.'.*.'.$this->extensionPorDefecto, FilesystemIterator::KEY_AS_FILENAME);
     ...
     */
    public function Log($identificador, $data, $incremental = true)
    {
    	// Antes que nada normalizo el identificador, que no pude contener puntos.
    	// Los puntos se reemplazan por guion bajo.
		...
		if ($incremental)
			$archivoLog = PATH_KRAKEN_LOG.$this->ObtenerArchivoSiguiente($id);
		else
			$archivoLog = PATH_KRAKEN_LOG.$id.'.'.$this->extensionPorDefecto;

		// Guardo los datos al archivo de log
        $archivo = fopen($archivoLog, 'w');
    	fputs($archivo, print_r($data, true));
        fclose($archivo);
    }

<?php
/**
 * Capa de negocio base para funcionalidad de PasoActuacionConfirmarFirma.
 *
 * @author XXXX
 *
 */
class NGPasoActuacionConfirmarFirma extends NGPasoActuacionBase {
	...
	public function asignarDatosAPasoActuacion(Actuacion $actuacion, PasoActuacion $paso, Usuario $pUsuario)
	{
		...
		// Procesamiento segun tipo de documento
		switch ($paso->opciones['documento_a_firmar']['tipo_documento']) {
			case 'directo':
				// Obtengo el url_base del archivo a firmar y el resto de la ruta (para el URL)
				$paso->datos['archivo_preview'] = sprintf('%s%s',
					$paso->opciones['documento_a_firmar']['url_base_documento_a_firmar'],
					$paso->opciones['documento_a_firmar']['ruta_documento_a_firmar']
				);

				// Si el paso es 'directo', y a su vez la actuación esta asociada a un
				// expediente, me arriesgo a verificar si el archivo a firmar se encuentra
			...
			case 'desde_paso':
				...
				$transac = NG::transacActuaciones()->obtenerTransacActuacion(
					$paso->id_transaccion,
					$paso->opciones['documento_a_firmar']['id_paso']
				);

				$data = json_decode($transac->data, true); // Datos de la transaccion

				$archivo_a_firmar = $data[$paso->opciones['documento_a_firmar']['parametro_paso']];

				// Obtengo el url_base del archivo a firmar, y el nombre del archivo a
				// firmar de un paso previo (extrayendolo de la ruta fisica).
				$paso->datos['archivo_preview'] = sprintf('%s%s',
					$paso->opciones['documento_a_firmar']['url_base_documento_a_firmar'],
					basename($archivo_a_firmar)
				);

				// Verifico si el archivo tiene embebidos
				$paso->datos['es_embebido'] = PDFExtractor::get()->hasAttachments($archivo_a_firmar);
	...
	public function procesarPasoActuacion(Actuacion $actuacion, PasoActuacion $paso, Usuario $pUsuario, &$params)
	{
		...
		if ($paso->opciones['firma_obligatoria']) {
			if ($params['f_op_confirmacion'] != 1)
				$errores[] = 'Debe firmar el documento para poder continuar.';
		} else {
			if (!in_array($params['f_op_confirmacion'], [0, 1]))
				$errores[] = 'Debe definir si desea firmar o no el documento.';
		}

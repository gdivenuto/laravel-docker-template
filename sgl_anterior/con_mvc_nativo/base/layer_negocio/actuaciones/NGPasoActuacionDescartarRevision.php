<?php
/**
 * Capa de negocio base para funcionalidad de PasoActuacionDescartarRevision.
 *
 * @author XXXX
 *
 */
class NGPasoActuacionDescartarRevision extends NGPasoActuacionBase {
	...
	public function asignarDatosAPasoActuacion(Actuacion $actuacion, PasoActuacion $paso, Usuario $pUsuario)
	{
		...
		switch ($paso->opciones['documento_a_descartar']['tipo_documento']) {
			case 'directo':
				// Obtengo el url_base del archivo y el resto de la ruta (para el URL)
				$paso->datos['archivo_preview'] = sprintf('%s%s',
					$paso->opciones['documento_a_descartar']['url_base_documento_a_descartar'],
					$paso->opciones['documento_a_descartar']['ruta_documento_a_descartar']
				);

				// Si el paso es 'directo', y a su vez la actuación esta asociada a un
				// expediente, me arriesgo a verificar si el archivo a descartar se encuentra
			...
			case 'desde_paso':
				...
				$transac = NG::transacActuaciones()->obtenerTransacActuacion(
					$paso->id_transaccion,
					$paso->opciones['documento_a_descartar']['id_paso']
				);

				$archivo_a_descartar = $data[$paso->opciones['documento_a_descartar']['parametro_paso']];

				// Obtengo el url_base del archivo, y el nombre del archivo a
				// firmar de un paso previo (extrayendolo de la ruta fisica).
				$paso->datos['archivo_preview'] = sprintf('%s%s',
					$paso->opciones['documento_a_descartar']['url_base_documento_a_descartar'],
					basename($archivo_a_descartar)
				);

				// Verifico si el archivo tiene embebidos
				$paso->datos['es_embebido'] = PDFExtractor::get()->hasAttachments($archivo_a_descartar);
	...
	public function procesarPasoActuacion(Actuacion $actuacion, PasoActuacion $paso, Usuario $pUsuario, &$params)
	{
		...
		$errores = $this->verificarExistenciaParametros(['f_op_descartar'], $params);
		if (count($errores) > 0) return $errores;

		if ($params['f_op_descartar'] != 1)
			$errores[] = 'Debe descartar el documento para poder continuar.';

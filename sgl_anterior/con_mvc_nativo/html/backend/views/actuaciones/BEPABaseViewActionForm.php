<?php
/**
 * Vista que renderiza un Editor de texto para la confección de un documento
 * 
 ...
 */
class BEPABaseViewActionForm extends BaseViewActionForm {
	// ************************************************************************
	...
	public function generarHtmlHeaderJS()
	{
		...
		// Agrego el JS común para todos los pasos
		echo '<script type="text/javascript" src="'.$this->vista->baseUrl.'js/actuaciones/paso_common.js?v='.SGL_BUILD_NUMBER.'"></script>'."\n";
	}
	...
	public function generarCabecera()
	{
		...
		// Agrego la cabecera básica para todos los pasos de actuacion		
		echo "\n<!-- ".get_class($this).".generarCabecera() -->\n";
		require($this->vista->baseTemplatePath.'actuaciones/base_cabecera_paso.php');
	}
	...
	{
		// Heredo comportamiento de BaseViewActionForm
		//parent::generarCuerpo();
		echo "\n<!-- ".get_class($this).".generarCuerpo() -->\n";
		require($this->vista->baseTemplatePath.'actuaciones/base_cuerpo_paso.php');
	}

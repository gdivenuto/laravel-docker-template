...
var listenerBotonGuardar = function () {
    ...
    if (formEdicionSolicitud.valid()) {
        ...
        $.ajax({
            ...
            url: "index.php?c=solicitudes&a=save&f_nuevo_estado=AEE&f_nueva_fecha_hora="+$('#f_nueva_fecha').val()+'%20'+$('#f_nueva_hora').val(),
            dataType: 'json', 
        ...
        .done(function( respuesta ) {
            if (respuesta.estado == 'OK') {
                if (respuesta.data != null) {
                    // Se muestra la grilla general de Préstamos
                    $(location).attr('href','index.php?c=solicitudes&a=view&f_anio='+respuesta.data.anio+'&f_tipo='+respuesta.data.tipo+'&f_numero='+respuesta.data.numero);
                } else
...
$(document).ready(function () {
    ...
    formEdicionSolicitud.validate({
        ...
        messages: {
                f_v_nueva_fecha: { 
                    required: "Debe especificar la fecha que se anula la solicitud."
                }
            },

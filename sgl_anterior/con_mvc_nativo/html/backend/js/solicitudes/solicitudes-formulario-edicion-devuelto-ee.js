...
var listenerBotonGuardar = function () {
    ...
    if (formEdicionSolicitud.valid()) {
        ...
        $.ajax({
            ...
            url: "index.php?c=solicitudes&a=save&f_generar_nueva_solicitud="+$('#f_generar_nueva_solicitud').val()+"&f_nuevo_estado=DEE&f_nueva_fecha_hora="+$('#f_nueva_fecha').val()+'%20'+$('#f_nueva_hora').val(),
            dataType: 'json', 
        ...
        .done(function( respuesta ) {
            if (respuesta.estado == 'OK') {
                if (respuesta.data != null) {
                    // Se muestra la grilla con la Solicitud devuelta
...
$(document).ready(function () {
    ...
    formEdicionSolicitud.validate({
        ...
        messages: {
                f_v_nueva_fecha: { 
                    required: "Debe especificar la fecha que se devuelve la solicitud."
                }
            },

...
var callbackRenderAcciones = function (data, type, full, meta) {
	...
	buttonAction += generarHtmlBotonAccion('enviarrecordatorio', 'Recordar la firma del documento', 'envelope', full, 'green');
...
var listenerBotonAccion = function (event) {
	...
	else if (accion == 'enviarrecordatorio') {

		if( confirm('¿Desea enviar un recordatorio de firma pendiente?') )
		{
			let item = dataTableRef.row($(this).parents('tr')).data();

			let url  = `index.php?c=firmaspendientessupervisor&a=enviarrecordatorio`;
				url += `&f_anio=${f_anio}&f_tipo=${f_tipo}&f_numero=${f_numero}&f_cuerpo=${f_cuerpo}&f_alcance=${f_alcance}`;
				url += `&f_mail_usuario_solicitante=${item.ro_mail_usuario_solicitante}`;
				url += `&f_nombre_usuario_solicitante=${item.ro_nombre_usuario_solicitante}`;
				url += `&f_mail_usuario=${item.ro_mail_usuario}`;
				url += `&f_orden=${f_orden}`;
...
function setDataTable(ajaxUrl) {
	...
	$(idTablaContainer).append(
		generarGrillaHtml(
			idTabla, 
			['Acciones', 'Documento', 'Detalle', 'Solicitante de firma', 'Firmante pendiente', 'Observaciones', 'Pendiente desde', 'D&iacute;as pendiente']
		)
	);
	...
	var tabla = $(idTabla)
		...
		.DataTable({
			...
			columns: [
				...
				{data: 'ro_detalle', render: callbackRenderDetalle}, //, width: "40%"
				{data: 'ro_nombre_usuario_solicitante'},
				{data: 'ro_nombre_usuario'},
				{data: 'ro_observaciones_ee', render: callbackRenderObservaciones},
				{data: 'fecha_hora_entrada'},
				{data: 'ro_dias_pendiente', className: 'text-center'},
			]
		});
...
$(document).ready(function() {
	...
	// Genero la tabla
	dataTableRef = setDataTable('index.php?c=firmaspendientessupervisor&a=datagrid');

...
confirmarPasoActual = function () {
    ...
    if ( ! verificarCheckbox('.op_revision') )
        errores.push((paso.opciones['revisar_y_firmar'])
            ? 'Debe definir si el documento pendiente es "confirmado y firmado" o "rechazado".'
            : 'Debe definir si el documento pendiente es "confirmado" o "rechazado".'
        );
    
    // Hago una verificacion forzada de SI y NO para asegurarme que
    // solamente haya una única opcion seleccionada.
    if ( ($('#op_revision_si').prop('checked') && $('#op_revision_no').prop('checked')) ||
         (!$('#op_revision_si').prop('checked') && !$('#op_revision_no').prop('checked')))
...
recuperarTransaccion = function () {
    ...
    if (transaccion.f_op_revision == 1) {
        $('#op_revision_si').prop('checked', true);
        $('#op_revision_no').prop('checked', false);
    }
    else
        // Si NO se había confirmado la firma
        if (transaccion.f_op_revision == 0) {
            $('#op_revision_si').prop('checked', false);
            $('#op_revision_no').prop('checked', true);
        }

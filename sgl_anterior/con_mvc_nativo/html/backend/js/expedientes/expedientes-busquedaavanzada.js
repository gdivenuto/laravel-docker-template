...
var callbackRenderFichas = function (data, type, full, meta) {
    ...
    if (full != null)
    {
        ...
        if (full.estados._items.length > 0){
            ...
            ficha += '<div class="row">';
            ficha +=     '<div class="col-sm-6 col-md-6"><strong>Estado: </strong>'+estado_actual.id_codestado+' - '+estado_actual.ro_nombre_estado+'</div>';
            ficha +=     '<div class="col-sm-6 col-md-3"><strong>Fecha de Estado: </strong>'+fecha_estado+'</div>';
            ficha += '</div>';
        ...
        if (full.antecedentes._items.length > 0){
            ...
            ficha += '<div class="row">';
            ficha +=     '<div class="col-md-1"><strong>Antecedente: </strong></div>';
            ficha +=     '<div class="col-md-11">{0} - {1} - {2} - {3} - {4} - {5}</div>'.format(antecedente.tipo_a, antecedente.numero_a, antecedente.digito_a, antecedente.anio_a, antecedente.cuerpo_a, antecedente.alcance_a);
            ficha += '</div>';
        }
        /*********************************************************/
    }
...
var listenerBotonRefrescar = function () {
    ...
    $(location).attr('href','index.php?c=expedientesbusquedaavanzada&a=view&f_restablecer=1');
};
        ...
        .DataTable({
    		...
    		ajax: {
                ...
                data: function ( d ) {
                    ...
                    d.f_caratula = $('#f_caratula').val();
                    d.f_iniciador = $('#f_iniciador').val();
                    d.f_categoria = $('#f_categoria').val();
                    d.f_comision = $('#f_comision').val();
                    d.f_estado = $('#f_estado').val();
                    d.f_autor = $('#f_autor').val();
                    d.f_tema = $('#f_tema').val();
                    d.f_opcion_sancionado_promulgado = $('#f_opcion_sancionado_promulgado').val();// Todos|Sancionados|Promulgados
                }
            },
            ...
            drawCallback: function(opciones){
                ...
                $('#ba_cantidad_resultados').html(tableinfo.recordsTotal);
                ...
                if (tableinfo.recordsTotal > 0) {
                    ...
                    $('#btn_exportar_planilla').prop('disabled', false);
                }
                else // Si NO hay para mostrar
                {
                    ...
                    $('#btn_exportar_planilla').prop('disabled', true);
                }
            }
    	});
...
/**
 * Se visualiza una modal para buscar un Iniciador, utilizando un autosugerido
 */
function mostrarModalIniciadorAutosugerido() {
    // Se muestra una modal para buscar un Iniciador mediante un autosugerido
    $('#modalIniciadorAutosugerido').modal('show');
    // Se limpia el campo de búsqueda
    $('#modal_iniciador_sugerido').val('');
    // Se selecciona y se le da el foco al campo de búsqueda por autosugerido
    setfocus('#modal_iniciador_sugerido');
}
...
/**
 * Se visualiza una modal para buscar una Categoría, utilizando un autosugerido
 */
function mostrarModalCategoriaAutosugerida() {
    // Se muestra una modal para buscar una Categoría mediante un autosugerido
    $('#modalCategoriaAutosugerida').modal('show');
    // Se limpia el campo de búsqueda
    $('#modal_categoria_sugerida').val('');
    // Se selecciona y se le da el foco al campo de búsqueda por autosugerido
    setfocus('#modal_categoria_sugerida');
}

/**
 * Se visualiza una modal para buscar un Autor, utilizando un autosugerido
 */
function mostrarModalAutorAutosugerido() {
    // Se muestra una modal para buscar un Autor mediante un autosugerido
    $('#modalAutorAutosugerido').modal('show');
    // Se limpia el campo de búsqueda
    $('#modal_autor_sugerido').val('');
    // Se selecciona y se le da el foco al campo de búsqueda por autosugerido
    setfocus('#modal_autor_sugerido');
}

/**
 * Se visualiza una modal para buscar un Tema, utilizando un autosugerido
 */
function mostrarModalTemaAutosugerido() {
    // Se muestra una modal para buscar un Tema mediante un autosugerido
    $('#modalTemaAutosugerido').modal('show');
    // Se limpia el campo de búsqueda
    $('#modal_tema_sugerido').val('');
    // Se selecciona y se le da el foco al campo de búsqueda por autosugerido
    setfocus('#modal_tema_sugerido');
}
...
$(document).ready(function () {
    ...
    $('#v_fecha_desde').datepicker({ altField: '#f_fecha_desde',
                                     ...
                                     onChange: function(fecha_elegida) {
                                        // Se establece como valor mínimo de la fecha hasta 
                                        $("#v_fecha_hasta").datepicker( "option", "minDate", fecha_elegida);
                                     }
                                   });
    ...
    // Al utilizar el combo de Todos/Sancionados/Promulgados
    $('#f_opcion_sancionado_promulgado').change(function () {
        // Al elegir por "Sólo Sancionados" ó "Sólo Promulgados"
        if ( $('#f_opcion_sancionado_promulgado').val() != 0 ) {
            // Se deshabilitan los combos de Comisiones y Estados
            $('#f_comision').prop('disabled', true);
            $('#f_estado').prop('disabled', true);
        } else {
            // Se habilitan los combos de Comisiones y Estados
            $('#f_comision').prop('disabled', false);
            $('#f_estado').prop('disabled', false);
        }
    });

	// **************** Comportamiento de botones ************************************
    $('#btn_buscar').click(function () {
        var fecha_desde = $('#v_fecha_desde').val();
        var fecha_hasta = $('#v_fecha_hasta').val();

        // Si la fecha Desde ES MAYOR a la fecha Hasta
        if ( fecha_desde != '' && fecha_hasta != '' && ! compararFechas(fecha_desde, fecha_hasta) ) {
            alert("La fecha Desde debe ser menor a la fecha Hasta.");
        } else {
            if (formBusquedaRef.valid()) {
                dataTableRef = setDataTable('index.php?c=expedientesbusquedaavanzada&a=busquedaavanzadadatagrid');
    ...
    $('#btn_imprimir').click(function () { 
        ...
        if (formBusquedaRef.valid()) {
            // Se genera el pdf del resultado de la Búsqueda Avanzada
            var url = 'index.php?c=reportes&a=busquedaavanzada&f_fecha_desde={0}&f_fecha_hasta={1}&f_caratula={2}&f_iniciador={3}&f_categoria={4}&f_comision={5}&f_estado={6}&f_autor={7}&f_tema={8}&f_opcion_sancionado_promulgado={9}'.format(
                $('#f_fecha_desde').val(),
                $('#f_fecha_hasta').val(),
                $('#f_caratula').val(),
                $('#f_iniciador').val(),
                ...
                $('#f_autor').val(),
                $('#f_tema').val(),
                $('#f_opcion_sancionado_promulgado').val()
            );
            // Se muestra el pdf en una nueva pestaña
            window.open(url);
        }
    ...
    $('#btn_exportar_texto').click(function () { 
        // Se validan los campos requeridos del form del criterio de búsqueda
        if (formBusquedaRef.valid()) {
            // Se genera el documento de texto del resultado de la Búsqueda Avanzada
            $(location).attr('href','index.php?c=reportes&a=busquedaavanzadadocumentotexto&f_fecha_desde={0}&f_fecha_hasta={1}&f_caratula={2}&f_iniciador={3}&f_categoria={4}&f_comision={5}&f_estado={6}&f_autor={7}&f_tema={8}&f_opcion_sancionado_promulgado={9}'.format(
                $('#f_fecha_desde').val(),
                $('#f_fecha_hasta').val(),
                $('#f_caratula').val(),
                $('#f_iniciador').val(),
    ...
    $('#btn_exportar_planilla').click(function () { 
        // Se validan los campos requeridos del form del criterio de búsqueda
        if (formBusquedaRef.valid()) {
            // Se genera la planilla de cálculo del resultado de la Búsqueda Avanzada
            $(location).attr('href','index.php?c=reportes&a=busquedaavanzadaplanillacalculo&f_fecha_desde={0}&f_fecha_hasta={1}&f_caratula={2}&f_iniciador={3}&f_categoria={4}&f_comision={5}&f_estado={6}&f_autor={7}&f_tema={8}&f_opcion_sancionado_promulgado={9}'.format(
                $('#f_fecha_desde').val(),
                $('#f_fecha_hasta').val(),
                $('#f_caratula').val(),
                $('#f_iniciador').val(),
    ...
    $('#modal_iniciador_sugerido').keypress( function(e) {
        if ( $('#modal_iniciador_sugerido').val() != '' && e.which == 13 )
            $('#btCargarIniciadorSugerido').focus();
    });
    ...
    $('#modal_categoria_sugerida').keypress( function(e) {
        if ( $('#modal_categoria_sugerida').val() != '' && e.which == 13 )
            $('#btCargarCategoriaSugerida').focus();
    });

    $('#modal_autor_sugerido').keypress( function(e) {
        if ( $('#modal_autor_sugerido').val() != '' && e.which == 13 )
            $('#btCargarAutorSugerido').focus();
    });

    $('#modal_tema_sugerido').keypress( function(e) {
        if ( $('#modal_tema_sugerido').val() != '' && e.which == 13 )
            $('#btCargarTemaSugerido').focus();
    });

    $('#btCargarIniciadorSugerido').click(function() {
        if ( $('#modal_iniciador_sugerido').val() != '' ) {
            // Se toma el Iniciador sugerido y elegido
            var iniciador_sugerido = $('#modal_iniciador_sugerido').val();
            // Se separa el tipo, el código y la descripción
            var aux_iniciador = iniciador_sugerido.split('-');
            // Se toma el código
            var codigo_iniciador = aux_iniciador[0]+'|'+aux_iniciador[1];
            // Se selecciona el Iniciador en el combo
            $('#f_iniciador').val(codigo_iniciador);
            // Se oculta la modal
            $('#modalIniciadorAutosugerido').modal('hide');
        } else
            setfocus('#modal_iniciador_sugerido');
    });
    ...
    $('#btCargarCategoriaSugerida').click(function() {
        if ( $('#modal_categoria_sugerida').val() != '' ) {
            // Se toma la Categoría sugerida y elegida
            var categoria_sugerida = $('#modal_categoria_sugerida').val();
            // Se separa el código y la descripción
            var aux_categoria = categoria_sugerida.split('-');
            // Se toma el código
            var codigo_categoria = aux_categoria[0];
            // Se selecciona la Categoría en el combo
            $('#f_categoria').val(codigo_categoria);
            // Se oculta la modal
            $('#modalCategoriaAutosugerida').modal('hide');
        } else
            setfocus('#modal_categoria_sugerida');
    });

    $('#btCargarAutorSugerido').click(function() {
        if ( $('#modal_autor_sugerido').val() != '' ) {
            // Se toma el Autor sugerido y elegido
            var autor_sugerido = $('#modal_autor_sugerido').val();
            // Se separa el tipo, el código y la descripción
            var aux_autor = autor_sugerido.split('-');
            // Se toma el código
            var codigo_autor = aux_autor[0]+'|'+aux_autor[1];
            // Se selecciona el Autor en el combo
            $('#f_autor').val(codigo_autor);
            // Se oculta la modal
            $('#modalAutorAutosugerido').modal('hide');
        } else
            setfocus('#modal_autor_sugerido');
    });

    $('#btCargarTemaSugerido').click(function() {
        if ( $('#modal_tema_sugerido').val() != '' ) {
            // Se toma el Tema sugerido y elegido
            var tema_sugerido = $('#modal_tema_sugerido').val();
            // Se separa el código y el nombre
            var aux_tema = tema_sugerido.split('-');
            // Se toma el código
            var codigo_tema = aux_tema[0];
            // Se selecciona el Estado en el combo
            $('#f_tema').val(codigo_tema);
            // Se oculta la modal
            $('#modalTemaAutosugerido').modal('hide');
        } else
            setfocus('#modal_tema_sugerido');
    });
    ...
    // Validación del formulario "form_busqueda_avanzada"
    formBusquedaRef = $("#form_busqueda_avanzada");

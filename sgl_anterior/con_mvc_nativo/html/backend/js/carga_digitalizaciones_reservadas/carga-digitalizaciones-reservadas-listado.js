	...
	showModal('Confirmaci&oacute;n', '¿Est&aacute; seguro que desea eliminar la digitalizaci&oacute;n temporal para el Expediente: {0}-{1}-{2}?'.format(expe_anio, expe_tipo, expe_numero),
	{
		btn_si: {
			action: function (e) {
				...
				$.ajax({
					method: "POST",
					url: "index.php?c=cargadigitalizacionesreservadas&a=eliminardigitalizaciontemporal",
...
function cargarDigitalizacionTemporal(f_archivo) {
    ...
    $.ajax({
        ...
        url: "index.php?c=cargadigitalizacionesreservadas&a=cargardigitalizaciones",
        dataType: 'json', 
...
function consultarAlUsuario(digi_existente) {
	...
	showModal(
		'Atenci&oacute;n', 
		'La digitalizaci&oacute;n reservada <strong>'+digi_existente+'</strong> ya existe ¿desea Sobreescribirla o Agregarla (unirla) a la existente?',
		{
...
function sobreescribirDigitalizacion(digi) {
    ...
    $.ajax({
        method: "POST",
        contentType: "application/json", // indicamos que enviamos un JSON como parametro (cuerpo de la peticion)
        url: "index.php?c=cargadigitalizacionesreservadas&a=sobreescribirdigitalizacion",
        dataType: 'json', 
...
function agregarDigitalizacion(digi) {
    ...
    $.ajax({
        method: "POST",
        contentType: "application/json", // indicamos que enviamos un JSON como parametro (cuerpo de la peticion)
        url: "index.php?c=cargadigitalizacionesreservadas&a=agregardigitalizacion",
        dataType: 'json', 
        data: JSON.stringify({ digitalizacion: digi }) // cadena json con: el nombre completo de la digitalización (extensión pdf incluída)
    })
...
$(document).ready(function() {
	...
	dataTableDocumentos = setDataTable('index.php?c=cargadigitalizacionesreservadas&a=datagriddocall');

...
var callbackRenderFichas = function (data, type, full, meta) {
    ...
    if (full != null)
    {
        if (full.tipo == 'E') nombre_documento = 'Expediente';
        if (full.tipo == 'N') nombre_documento = 'Nota';
        if (full.tipo == 'R') nombre_documento = 'Recomendaci&oacute;n';
        ...
        ficha +=            '<button type="button" class="btn btn-primary btn-sm" onClick="javascript:irA(\'expedientes\',{0},\'{1}\',{2},{3},{4});" title="Ir al expediente"><span class="glyphicon glyphicon-share-alt"></span>&nbsp;Ir al expediente</button>'.format(full.anio, full.tipo, full.numero, full.cuerpo, full.alcance);
        ficha +=            '&nbsp;&nbsp;&nbsp;';
...
var listenerBotonRefrescar = function () {
    ...
    $(location).attr('href','index.php?c=listadoordendeldia&a=view&f_restablecer=1');
};
...
$(document).ready(function () {
    ...
    $('#btn_buscar').click(function () {
        ...
        if (formBusquedaRef.valid()) {
            ...
            if ( $('#f_comision').val() == 0 )
                ...
                dataTableRef = setDataTable('index.php?c=listadoordendeldia&a=ordendeldiadatagrid');
    ...
    $('#btn_imprimir').click(function () { 
        ...
        if (formBusquedaRef.valid()) {
            // Se genera el PDF del reporte de Orden del Día
            var url = 'index.php?c=listadoordendeldia&a=generarpdfordendeldia&f_fecha_desde={0}&f_fecha_hasta={1}&f_comision={2}'.format(
                $('#f_fecha_desde').val(),
                $('#f_fecha_hasta').val(),
                $('#f_comision').val()
            );
    ...
    $('#btn_exportar_texto').click(function () { 
        // Se validan los campos requeridos del form del criterio de búsqueda
        if (formBusquedaRef.valid()) {
            // Se genera el documento de texto del reporte de Orden del Día
            $(location).attr('href','index.php?c=listadoordendeldia&a=generardocumentotextoordendeldia&f_fecha_desde={0}&f_fecha_hasta={1}&f_comision={2}'.format(
                $('#f_fecha_desde').val(),
                $('#f_fecha_hasta').val(),
                $('#f_comision').val()
            ));
    ...
    formBusquedaRef = $("#form_orden_del_dia");

...
var listenerBotonGuardar = function () {
    ...
    if (formEdicionPrestamo.valid()) {
        ...
        $.ajax({
            ...
            url: "index.php?c=prestamos&a=save&f_grilla="+$('#f_grilla').val()+"&f_nuevo_estado=P&f_nueva_fecha_hora="+$('#f_nueva_fecha').val()+'%20'+$('#f_nueva_hora').val(),
            dataType: 'json', 
...
{
    prestamo.libro_numero   = $('#f_libro_numero').val();
    prestamo.libro_folio    = $('#f_libro_folio').val();
...
function asignarPrestamoAFormulario()
{
    ...
    $('#f_libro_numero').val(decodeEntities(prestamo.libro_numero));
    $('#f_libro_folio').val(decodeEntities(prestamo.libro_folio));
...
$(document).ready(function () {
    ...
    formEdicionPrestamo.validate({
        rules: {
                ...
                f_libro_numero: { required: true, digits: true },
                f_libro_folio: { required: true, digits: true }
               },
        messages: {
                f_v_nueva_fecha: { 
                    required: "Debe especificar la fecha que se presta el expediente."
                },
                f_libro_numero: { 
                    required: "Por favor ingrese el n"+u_acentuada+"mero del libro.",
                    digits: "Por favor ingrese s"+o_acentuada+"lo n"+u_acentuada+"meros."
                },
                f_libro_folio: { 
                    required: "Por favor ingrese el folio del libro.",
                    digits: "Por favor ingrese s"+o_acentuada+"lo n"+u_acentuada+"meros."
                }
            },

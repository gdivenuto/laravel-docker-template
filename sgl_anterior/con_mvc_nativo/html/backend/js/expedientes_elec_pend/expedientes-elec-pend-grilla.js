...
var callbackRenderAcciones = function (data, type, full, meta) {
	...
	if (id_usuario == full.ro_id_usuario_solicitante)
		buttonAction += generarHtmlBotonAccion('descartar_revision', 'Descartar la revisión del documento', 'trash', full)
	else
		buttonAction += generarHtmlSoloIcono('Sólo el solicitante puede descartar la revisión del documento', 'trash', full, 'lightgray');
	...
	if (full.id_usuario == id_usuario) 
		buttonAction += generarHtmlBotonAccion('confirmar', 'Confirmar el documento pendiente.', 'ok', full, 'green');
...
var listenerBotonAccion = function (event) {
	...
	if (accion == 'view') {
	...
	} else if (accion == 'descartar_revision') {
		iniciarActuacion('expe_elec_pend_descartar', {anio: f_anio, tipo: f_tipo, numero: f_numero, cuerpo: f_cuerpo, alcance: f_alcance, orden: f_orden, id_revision: f_id_revision});
		...
		iniciarActuacion('expe_elec_pend_confirmar', {anio: f_anio, tipo: f_tipo, numero: f_numero, cuerpo: f_cuerpo, alcance: f_alcance, orden: f_orden, id_revision: f_id_revision});
...
function setDataTable(ajaxUrl) {
	...
	idTabla = '#grillaRevisionesPendientes';
	idTablaContainer = '{0}Container'.format(idTabla);

	$(idTablaContainer).empty();
	$(idTablaContainer).append(generarGrillaHtml(idTabla, ['Acciones', 'Expediente', 'Detalle', 'Cargado por', 'Confirma', 'Observaciones', 'Pendiente desde']));
...
$(document).ready(function() {
	controlador_activo = 'expedienteselecpend';
	...
	// Genero la tabla
	dataTableRef = setDataTable('index.php?c=expedienteselecpend&a=datagrid&f_detalle='+$('#f_detalle').val());

...
function generarHtmlBotonAccion(accion, descripcion, icono, fila) {
	return '<span class="btn-accion-contenido glyphicon glyphicon-{2}" data-accion="{0}" title="{1}" data-anio="{3}" data-tipo="{4}" data-numero="{5}" data-cuerpo="{6}" data-alcance="{7}" data-orden_proyecto="{8}" data-fecha_sancion="{9}" ></span>&nbsp;&nbsp;'.format(
		accion, descripcion, icono,
		fila.anio, fila.tipo, fila.numero, fila.cuerpo, fila.alcance, fila.orden_proyecto, fila.fecha_sancion);
}
...
var callbackRenderAcciones = function (data, type, full, meta) {
	...
	if (perfil_usuario_actual == 1 || perfil_usuario_actual == 2) {
    	buttonAction += generarHtmlBotonAccion('edit', 'Editar Sanci&oacute;n', 'pencil', full);
    	buttonAction += generarHtmlBotonAccion('delete', 'Eliminar Sanci&oacute;n', 'trash', full);
	}

    // // Icono para determinar si esta promulgado o vetado
    // if ((full.fecha_promulga != null) && (full.fecha_promulga != ''))
    // 	buttonAction += '<span class="glyphicon glyphicon-thumbs-up forzar_texto_color_verde" title="Promulgado"></span>';
    // else if ((full.fecha_veto != null) && (full.fecha_veto != ''))
    // 	buttonAction += '<span class="glyphicon glyphicon-thumbs-down forzar_texto_color_rojo" title="Vetado"></span>';
...
var listenerBotonAccion = function (event) {
	...
	f_fecha_sancion = $(this).data('fecha_sancion');

	if (accion == 'edit')
		$(location).attr('href','index.php?c=sanciones&a=edit&f_anio={0}&f_tipo={1}&f_numero={2}&f_cuerpo={3}&f_alcance={4}&f_orden_proyecto={5}&f_fecha_sancion={6}'.format(
			f_anio, f_tipo, f_numero, f_cuerpo, f_alcance, f_orden_proyecto, f_fecha_sancion ));
	else if (accion == 'delete') {
		item = dataTableRef.row($(this).parents('tr')).data();
		eliminarSancion(item);
	}
	else
		showModal('Aviso', 'Acci&oacute;n no definida: \'{0}\'. Sanci&oacute;n del Expediente: {1}-{2}-{3}-{4}-{5} Orden {6} Fecha {7}'.format(accion, f_anio, f_tipo, f_numero, f_cuerpo, f_alcance, f_orden_proyecto, f_fecha_sancion));
};

/**
 * Se envía la Sanción respectiva para su eliminación
 * @param  {[type]} sancion [description]
 * @return {[type]}            [description]
 */
function eliminarSancion (sancion) {
	showModal('Atenci&oacute;n', '¿Est&aacute; seguro que desea eliminar la Sanci&oacute;n del Expediente: {0}-{1}-{2}-{3}-{4}?'.format(sancion.anio, sancion.tipo, sancion.numero, sancion.cuerpo, sancion.alcance),
	{
		btn_si: {
			action: function (e) {
				...
				$.ajax({
					...
					url: "index.php?c=sanciones&a=delete",
					dataType: 'json',
					data: JSON.stringify(sancion)
				})
				.done(function( respuesta ) {
					if (respuesta.estado == 'OK') {
						if ((respuesta.data != null) && (typeof respuesta.data.sancion != 'undefined'))
							// Refrescar grilla
							dataTableRef.ajax.reload(null, false);
						else
							showModal('Error', 'Se esperaba una Sanción y no se recibieron resultados.');
					} else
...
var callbackDrawCallback = function(settings) {
	...
	if ( settings.json.existeExpediente)
		$('#btn_nueva_sancion').prop('disabled', false); // Se habilita el botón 'Nueva Sanción'
	else // si NO existe
		$('#btn_nueva_sancion').prop('disabled', true); // Se deshabilita el botón 'Nueva Sanción'
};
...
function definirColumnasGrilla() {
	...
	if (perfil_usuario_actual == 1 || perfil_usuario_actual == 2)
		...
		columnas_grilla = [
							...
							{data: 'numero_sancion'},
							{data: 'fecha_sancion', className:'text-center', render: function (data, type, full, meta) { return formatearFechaConBarras(full.fecha_sancion); }},
							{data: 'numero_promulga'},
							{data: 'fecha_promulga', className:'text-center', render: function (data, type, full, meta) { return formatearFechaConBarras(full.fecha_promulga); }},
							{data: 'decreto_promulga'},
							{data: 'fecha_veto', className:'text-center', render: function (data, type, full, meta) { return formatearFechaConBarras(full.fecha_veto); }}
					      ];
	else
		columnas_grilla = [
							{data: 'orden_proyecto'},
...
$(document).ready(function() {
	...
	// Genero la tabla
	dataTableRef = setDataTable('index.php?c=sanciones&a=datagrid&f_anio='+$('#f_anio').val()+'&f_tipo='+$('#f_tipo').val()+'&f_numero='+$('#f_numero').val()+'&f_cuerpo='+$('#f_cuerpo').val()+'&f_alcance='+$('#f_alcance').val());
	...
	$('#btn_nueva_sancion').click(function () {
		$(location).attr('href','index.php?c=sanciones&a=add&f_anio='+$('#f_anio').val()+'&f_tipo='+$('#f_tipo').val()+'&f_numero='+$('#f_numero').val()+'&f_cuerpo='+$('#f_cuerpo').val()+'&f_alcance='+$('#f_alcance').val());
	});
	...
	$('#solapa_sanciones').addClass('active');

	// 09/09/2020 XXXX
	// ---------------------
	// No son necesarios en la solapa de Sanciones

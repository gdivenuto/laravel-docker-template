<?php
/**
 * Clase de controlador del listado de Expedientes sin documento cargado.
 *
 * @author XXXX, XXXX
 */
class BEListadoexpedientessindocumentocargadoController extends BaseController
{
	// ************************************************************************
	...
	public function __construct()
	{
		...
		$this->accionesPermitidas['expedientessindocumentocargadodatagrid'] = NIVEL_ACCESO_OPERADOR;
		$this->accionesPermitidas['generarpdfsindocumentocargado'] = NIVEL_ACCESO_OPERADOR;
		$this->accionesPermitidas['generardocumentotextosindocumentocargado'] = NIVEL_ACCESO_OPERADOR;
	...
	public function view($requestParams)
	{
		...
		$parametros_listado_sin_documento_cargado = $this->setearValoresPorDefectoParaVista();
		...
		if ($f_restablecer != '')
		{
			try {
				...
				SessionController::get()->eliminar('parametros_listado_sin_documento_cargado');
			}
			catch (Exception $ex) {
		...
		else // si NO se desea restablecer
		{
				...
				isset($requestParams['f_fecha_hasta']) && $requestParams['f_fecha_hasta'] != '' )
			{
				try	{
					...
					$parametros_listado_sin_documento_cargado = $this->obtenerParametros($requestParams);
				}
				catch (Exception $ex) {
					// Si falla, vuelve al home y elimina la busqueda en sesión
					SessionController::get()->eliminar('parametros_listado_sin_documento_cargado');
			...
			else // Si NO se recibieron las fechas desde y hasta
			{
				try {
					...
					if (SessionController::get()->existe('parametros_listado_sin_documento_cargado')) {
						// Se obtienen desde la sesión
						$parametros_listado_sin_documento_cargado = SessionController::get()->obtener('parametros_listado_sin_documento_cargado');
					}
				} catch (Exception $ex) {
					// Si falla, vuelve al home y elimina la busqueda en sesión
					SessionController::get()->eliminar('parametros_listado_sin_documento_cargado');
		...
		$paramVista['parametros_listado_sin_documento_cargado'] = $parametros_listado_sin_documento_cargado;

		// Se guardan en sesion (siempre... en un caso es redundante)
		SessionController::get()->guardar('parametros_listado_sin_documento_cargado', $parametros_listado_sin_documento_cargado);

		// Se ejecuta la Vista
		// Instancio la vista y la muestro
		$vista = new BEListadosView($paramVista);
		$vista->vistaListadoExpedientesSinDocumentoCargado();
	}
	 ...
	 * @param  mixed $requestParams Por claridad del modelo, se pasa por parámetro el conjunto de parametros de la petición (generalmente la union de $_REQUEST y $_FILES).
	 */
	public function expedientessindocumentocargadodatagrid($requestParams)
	{
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			$expedientes_sin_documentos_cargados = array();
			$resultado = array();
			...
			try {
					 ...
					 ($parametros['f_fecha_hasta'] == null || $parametros['f_fecha_hasta'] == '') )
				{
					...
					SessionController::get()->eliminar('parametros_expedientes_sin_documento_cargado');
				}
				else
				{
					...
					foreach ($expedientes as $e) {
						// Obtenemos el estado del proyecto del expediente
						NG::expedientes()->determinarEstadoProyecto($e);

						// Solamente nos quedamos con aquellos que NO poseen un documento cargado
						if ( $e->estado_proyecto === ESTADO_PROYECTO_SIN_CARGAR ) {
							$expedientes_sin_documentos_cargados[$pos] = $e;
							$pos++;
						}
					}

					// De los que quedaron, obtenemos los Proyectos, en caso que posea
					foreach ($expedientes_sin_documentos_cargados as $exp)
						$exp = NG::expedientes()->completarInstanciaSoloConProyectos($exp);

					// Resultado a mostrar en la Vista
					$paramVista['resultados'] = $expedientes_sin_documentos_cargados;

					// preparo el resultado (como no uso el filtro de datatables, Total y Filtered son iguales)
					$resultado['recordsTotal'] = count($expedientes_sin_documentos_cargados);
					$resultado['recordsFiltered'] = count($expedientes_sin_documentos_cargados);
					// Se asignan los expedientes obtenidos a la Vista
					$resultado['data'] = $expedientes_sin_documentos_cargados;
				}
			} catch (Exception $ex) {
				$resultado['recordsTotal'] = 0;
				$resultado['recordsFiltered'] = 0;
		...
		SessionController::get()->guardar('parametros_expedientes_sin_documento_cargado', $parametros);
	...
	/**
	 * Se obtienen los expedientes que NO posean un documento cargado, de un rango de fechas específico
	 * @param  [array] $parametros Array con la fecha Desde y la fecha Hasta
	 * @return [array]             Array con los Expedientes
	 */
	public function obtenerExpedientesSinDocumentoCargado($parametros)
	{
		...
		if ( $cantidad_auxiliar > 0 )
		{
			...
			// Por cada expediente obtenido, del listado completo
			 ...
			 foreach ($listado_completo as $e) {
				...
				// Solamente nos quedamos con aquellos que NO poseen un documento cargado
				if ( $e->estado_proyecto === ESTADO_PROYECTO_SIN_CARGAR ) {
					$expedientes_sin_documentos_cargados[$posicion] = $e;
					$posicion++;
				}
			}
		} else
			$expedientes_sin_documentos_cargados = array(); // Es un array vacio!!! No NULL

		return $expedientes_sin_documentos_cargados;
	}
	 ...
	 */
	public function generarpdfsindocumentocargado($requestParams) {
		// Antes que nada verifico el nivel de acceso.
		$this->verificarNivelAccesoUsuario(__FUNCTION__);
		...
		try {
			// Se obtienen los expedientes que NO posean un documento cargado, de un rango de fechas específico
			$expedientes_sin_documentos_cargados = $this->obtenerExpedientesSinDocumentoCargado($parametros);
			...
			$vista->vistaReportePdfSinDocumentoCargado();
		} catch (Exception $ex) {
			// Si falla, vuelve al home y elimina la busqueda en sesión
			SessionController::get()->guardarError($ex->getMessage(), ERROR_CONTROLLER_GENERICO);
			$this->redireccionar('home', 'view');
	 ...
	 * @param  array $requestParams Conjunto de parámetros
	 */
	public function generardocumentotextosindocumentocargado($requestParams) {
		// Antes que nada verifico el nivel de acceso.
		$this->verificarNivelAccesoUsuario(__FUNCTION__);
		...
		try {
			...
			$vista->vistaDocumentoTextosExpedientesSinDocumentoCargado();
		} catch (Exception $ex) {
			// Si falla, vuelve al home y elimina la busqueda en sesión
			SessionController::get()->guardarError($ex->getMessage(), ERROR_CONTROLLER_GENERICO);
			$this->redireccionar('home', 'view');

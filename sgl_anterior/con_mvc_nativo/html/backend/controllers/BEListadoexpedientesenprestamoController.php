<?php
/**
 * Clase de controlador del listado de Expedientes en Préstamo.
 *
 * @author XXXX, XXXX
 */
class BEListadoexpedientesenprestamoController extends BaseController
{
	// ************************************************************************
	...
	public function __construct()
	{
		...
		$this->accionesPermitidas['expedientesenprestamodatagrid'] = NIVEL_ACCESO_OPERADOR;
		$this->accionesPermitidas['generarpdfexpedientesenprestamo'] = NIVEL_ACCESO_OPERADOR;
		$this->accionesPermitidas['generardocumentotextoexpedientesenprestamo'] = NIVEL_ACCESO_OPERADOR;
		...
		$this->etiquetas['f_observaciones_estado'] = '&nbsp;&nbsp;&nbsp;Por observaciones del Estado';
	}
	...
	private function setearValoresPorDefectoParaVista()
	{
		...
		$resultado['f_fecha_desde'] 		 = '1983-01-01';
		$resultado['f_fecha_hasta'] 		 = date("Y-m-d");
		$resultado['f_estado'] 	    		 = 0;
		$resultado['f_observaciones_estado'] = '';
	...
	private function obtenerParametros($requestParams)
	{
		...
		$resultado['f_fecha_desde'] = (isset($requestParams['f_fecha_desde'])) ? $requestParams['f_fecha_desde'] : '1983-01-01';
		...
		// Si se recibe el texto para filtrar por observaciones del Estado
		$resultado['f_observaciones_estado'] = (isset($requestParams['f_observaciones_estado'])) ? $requestParams['f_observaciones_estado'] : null;
	...
	public function view($requestParams)
	{
		...
		$parametros_expedientes_en_prestamo = $this->setearValoresPorDefectoParaVista();
		...
		if ($f_restablecer != '')
		{
			try {
				...
				SessionController::get()->eliminar('parametros_expedientes_en_prestamo');
			}
			catch (Exception $ex) {
		...
		else // si NO se desea restablecer
		{
				...
				isset($requestParams['f_fecha_hasta']) && $requestParams['f_fecha_hasta'] != '' )
			{
				try	{
					...
					$parametros_expedientes_en_prestamo = $this->obtenerParametros($requestParams);
				}
				catch (Exception $ex) {
					// Si falla, vuelve al home y elimina la busqueda en sesión
					SessionController::get()->eliminar('parametros_expedientes_en_prestamo');
			...
			else // Si NO se recibieron las fechas desde y hasta
			{
				try {
					...
					if (SessionController::get()->existe('parametros_expedientes_en_prestamo')) {
						// Se obtienen desde la sesión
						$parametros_expedientes_en_prestamo = SessionController::get()->obtener('parametros_expedientes_en_prestamo');
					}
				} catch (Exception $ex) {
					// Si falla, vuelve al home y elimina la busqueda en sesión
					SessionController::get()->eliminar('parametros_expedientes_en_prestamo');
		...
		$paramVista['parametros_expedientes_en_prestamo'] = $parametros_expedientes_en_prestamo;

		// Se guardan en sesion (siempre... en un caso es redundante)
		SessionController::get()->guardar('parametros_expedientes_en_prestamo', $parametros_expedientes_en_prestamo);

		// Se ejecuta la Vista
		// Instancio la vista y la muestro
		$vista = new BEListadosView($paramVista);
		$vista->vistaListadoExpedientesEnPrestamo();
	}
	 ...
	 * @param  mixed $requestParams Por claridad del modelo, se pasa por parámetro el conjunto de parametros de la petición (generalmente la union de $_REQUEST y $_FILES).
	 */
	public function expedientesenprestamodatagrid($requestParams)
	{
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try {
				...
				else
				{
					...
					$parametros['f_observaciones_estado'] = ($parametros['f_observaciones_estado'] != '') ? Validator::get()->validar($parametros['f_observaciones_estado'], PATRON_ALFANUM_EXT, true, 'Observaciones del Estado') : null;
					...
					$expedientes = NG::reportes()->obtenerExpedientesEnPrestamo(
						// Parametros
				        $parametros['f_fecha_desde'],
						$parametros['f_fecha_hasta'],
						$parametros['f_estado'],
						$parametros['f_observaciones_estado'],
						true, // instancias completas!
						// Control de consulta
						null, // criterio y sentido de orden (FIJO), es NULO porque la query se arma a mano
					...
					$cantidadTotalExpedientes = NG::reportes()->obtenerExpedientesEnPrestamoCantidad(
						// Parametros
				        $parametros['f_fecha_desde'],
						$parametros['f_fecha_hasta'],
						$parametros['f_estado'],
		...
		SessionController::get()->guardar('parametros_expedientes_en_prestamo', $parametros);
	 ...
	 */
	public function generarpdfexpedientesenprestamo($requestParams) {
		// Antes que nada verifico el nivel de acceso.
		$this->verificarNivelAccesoUsuario(__FUNCTION__);
		...
		try {
			...
			$cantidadResultados = NG::reportes()->obtenerExpedientesEnPrestamoCantidad(
				// Parametros
		        $parametros['f_fecha_desde'],
				$parametros['f_fecha_hasta'],
				$parametros['f_estado'],
			...
			$expedientes = NG::reportes()->obtenerExpedientesEnPrestamo(
				...
				true, // instancias completas!
				// Control de consulta
				null // criterio y sentido de orden, es NULO porque la query se arma a mano
			);
			...
			$vista->vistaReportePdfExpedientesEnPrestamo();
		} catch (Exception $ex) {
			// Si falla, vuelve al home y elimina la busqueda en sesión
			SessionController::get()->guardarError($ex->getMessage(), ERROR_CONTROLLER_GENERICO);
			$this->redireccionar('home', 'view');
	 ...
	 * @param  array $requestParams Conjunto de parámetros
	 */
	public function generardocumentotextoexpedientesenprestamo($requestParams) {
		// Antes que nada verifico el nivel de acceso.
		$this->verificarNivelAccesoUsuario(__FUNCTION__);
		...
		try {
			...
			$vista->vistaDocumentoTextosExpedientesEnPrestamo();
		} catch (Exception $ex) {
			// Si falla, vuelve al home y elimina la busqueda en sesión
			SessionController::get()->guardarError($ex->getMessage(), ERROR_CONTROLLER_GENERICO);
			$this->redireccionar('home', 'view');

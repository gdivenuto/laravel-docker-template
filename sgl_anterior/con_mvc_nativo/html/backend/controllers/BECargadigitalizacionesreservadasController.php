<?php
/**
 * Clase de controlador para la Carga de Digitalizaciones Reservadas
 *
 * @author XXXX y XXXX
 */
class BECargaDigitalizacionesReservadasController extends BaseController {
	// ************************************************************************
	...
	public function view($requestParams) {
		...
		$vista = new BECargaDigitalizacionesReservadasView($paramVista);
		$vista->vistaListado();
	}
	...
	public function datagriddocall($requestParams) {
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try
			{
				...
				$documentos = NG::expedientes()->obtenerDigitalizacionesReservadasACargar();

				foreach ($documentos as $digi) {
					$digi['ruta_completa'] = '';
					// // Se descarta el Expediente, sin la letra "D" o "d" ó con la letra "A" o "a"
					// if (strripos($digi['archivo'], 'E') == '2' && (strripos($digi['archivo'], '.') == '8' || strripos($digi['archivo'], 'A') == '8')) {
					// 	$nada = '';
					// } else {
					// 	$auxiliar[] = $digi;
					// }
	...
	public function cargardigitalizaciones($requestParams) {
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try {
				...
				$resultadoProceso = NG::expedientes()->cargarDigitalizacionesReservadas($f_archivos);
	...
	public function agregardigitalizacion() {
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try {
				...
				$resultadoProceso = NG::expedientes()->agregarDigitalizacionReservada($f_digitalizacion);
	...
	public function sobreescribirdigitalizacion() {
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try {
				...
				$resultadoProceso = NG::expedientes()->sobreescribirDigitalizacionReservada($f_digitalizacion);
	...
	public function eliminardigitalizaciontemporal($requestParams) {
		...
		if (SessionController::get()->existe('MENSAJE_ERROR')) {
			...
			try {
				...
				NG::expedientes()->eliminarDigitalizacionReservadaTemporal($f_archivo);

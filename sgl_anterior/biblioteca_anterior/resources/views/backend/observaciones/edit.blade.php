...
<script>
    ...
    var fillData = function () {
        // Muestro resultados
        $.each(observaciones, function(k, v) {
            var f_row = addFormRow();
            f_row.find('.frm_id').val(v.id);
            f_row.find('.frm_obs').val(v.obs);
        });
    };
    ...
    var callbackBtnGuardar = function (e) {
        ...
        $('#form_data_table tr.form_data_row').each(function(i) { 
            ...
            var d_obs = $(this).find('.frm_obs').val();
            $('#dataForm').append(`<input type="text" name="observaciones[${i}][id]" value="${d_id}">`);
            $('#dataForm').append(`<input type="text" name="observaciones[${i}][obs]" value="${d_obs}">`);
        });
...
<template id="template_form_row">
    <tr class="form_data_row">
        ...
        <td><input type="text" class="form-control frm_obs" placeholder="Ingrese Observaciones"></td>
        <td><button type="button" class="btn btn-secondary btn_del_form_row"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td>
    </tr>    
</template>

<!-- form oculto -->
<form id="dataForm" class="invisible" action="{{ route('backend.observaciones.update', [ 'norma' => $norma ]) }}" method="POST">
    @method('put')
    @csrf
</form>
    ...
    <div class="row">
        <div class="col-sm-12">
            <table class="table" id="form_data_table">
                <thead>
                    <tr>
                        <th scope="col" style="width: 95%;">Observaciones</th>
                    ...
                    <tr>
                        <td colspan="2">
                            <button type="button" class="btn btn-secondary" id="add_form_row">
                                <i class="fa fa-plus" aria-hidden="true"></i> Agregar otra observaci&oacute;n
                            </button>
                        </td>
                    </tr>
                </tbody>

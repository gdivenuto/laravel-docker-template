@extends('backend.layouts.app')

@section('script')
<script src="{{ asset('js/i18n/'.\App::getLocale().'.js') }}"></script>
<script>
    // ---- Callbacks ---------------------------------------------------------
    var callbackRenderAcciones = function (data, type, row, meta) {
        var buttons = '';
        buttons += '<div class="btn-group" role="group" aria-label="Controles">';

        buttons += `<a role="button" class="btn btn-outline-secondary btn-sm" href="{{ route('backend.intendencias.edit', ['intendencia' => '__INTENDENCIA_ID__']) }}"><i class="fa fa-pencil" aria-hidden="true"></i></a>`
            .replace('__INTENDENCIA_ID__', data);

        buttons += `<a role="button" class="btn btn-outline-secondary btn-sm" href="{{ route('backend.intendencias.destroy', ['intendencia' => '__INTENDENCIA_ID__']) }}"><i class="fa fa-trash" aria-hidden="true"></i></a>`
            .replace('__INTENDENCIA_ID__', data);

        buttons += '</div>';
        return buttons;
    };

    var callbackRenderFecha = function (data, type, row, meta) {
        return (data) 
            ? moment(data).format('DD/MM/YYYY')
            : 'Actualidad';
    }

    // ------------------------------------------------------------------------
    // jQuery Document Ready Function -----------------------------------------
    // ------------------------------------------------------------------------
    $(function () {
        // Set moment locale
        moment.locale('{!! \App::getLocale() !!}');

        // Datatable config
	    $('#datagrid').DataTable({
            processing: true,
            serverSide: true,
            language: { url: '{{ asset(sprintf("json/datatables.%s.json", \App::getLocale())) }}' },
			dom: 
                "<'row'<'col-sm-12 col-md-3'l><'col-sm-12 col-md-3'f><'col-sm-12 col-md-6'p>>" +
                "<'row'<'col-sm-12'tr>>" +
                "<'row'<'col-sm-12 col-md-6'i><'col-sm-12 col-md-6'p>>",
            ajax: {
                url: '{{ route('backend.intendencias.dtgetintendenciasjson') }}'
            },
            //columnDefs: [
            //   { targets: [0, 2], orderable: true, searchable: false },
            //   { targets: [1, 3], orderable: false, searchable: false }
            //],
            columns: [
                { data: 'id',         name: 'id',         width:'5%', render: callbackRenderAcciones },
                { data: 'intendente', name: 'intendente', width:'40%' },
                { data: 'nro',        name: 'nro',        width:'15%' },
                { data: 'fec_desde',  name: 'fec_desde',  width:'20%', render: callbackRenderFecha },
                { data: 'fec_hasta',  name: 'fec_hasta',  width:'20%', render: callbackRenderFecha },
            ],
            pageLength: {{ config('app.backend_tableResultPerPage') }},
            order: [[3, 'asc']]
	    });
	});
</script>
@endsection

@section('content')
<div class="container-fluid">

    <div class="row">
        <div class="col-12">    
            <p class="h4">
                <i class="fa fa-home" aria-hidden="true"></i> &gt;
                <a href="{{ route('backend.dashboard.index') }}">Dashboard</a> &gt;
                Intendencias
            </p>
        </div>
    </div>
    
    @include('backend.layouts.errors')

    <div class="table-responsive-sm mt-2">
        <table id="datagrid" class="table table-striped table-sm table-hover">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Intendente</th>
                    <th>Nro</th>
                    <th>Fecha Desde</th>
                    <th>Fecha Hasta</th>
                </tr>
            </thead>
            <tbody>
            </tbody>
        </table>
    </div>
</div>
@endsection
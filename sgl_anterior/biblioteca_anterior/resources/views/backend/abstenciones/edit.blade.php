...
<script>
    // ---- Globals -----------------------------------------------------------
    var abstenciones = @json($abstenciones);

    // ---- Functions ---------------------------------------------------------
    var fillData = function () {
        // Muestro resultados
        $.each(abstenciones, function(k, v) {
    ...
    var callbackBtnGuardar = function (e) {
        ...
        $('#form_data_table tr.form_data_row').each(function(i) { 
            ...
            $('#dataForm').append(`<input type="text" name="abstenciones[${i}][id]" value="${d_id}">`);
            $('#dataForm').append(`<input type="text" name="abstenciones[${i}][nombre]" value="${d_nombre}">`);
        });
...
<template id="template_form_row">
    <tr class="form_data_row">
        ...
        <td><input type="text" class="form-control frm_nombre" placeholder="Ingrese Abstención"></td>
        <td><button type="button" class="btn btn-secondary btn_del_form_row"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td>
    </tr>    
</template>

<!-- form oculto -->
<form id="dataForm" class="invisible" action="{{ route('backend.abstenciones.update', [ 'norma' => $norma ]) }}" method="POST">
    @method('put')
    @csrf
</form>
    ...
    <div class="row">
        <div class="col-sm-12">
            <table class="table" id="form_data_table">
                <thead>
                    <tr>
                        <th scope="col" style="width: 95%;">Abstenci&oacute;n</th>
                    ...
                    <tr>
                        <td colspan="2">
                            <button type="button" class="btn btn-secondary" id="add_form_row">
                                <i class="fa fa-plus" aria-hidden="true"></i> Agregar otra abstenci&oacute;n
                            </button>
                        </td>
                    </tr>
                </tbody>
